...
export function Navigation() {
  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-50 bg-gray-100 px-4 py-3">
        {/* The floating pill nav */}
        <div
          ref={ref}
          className="max-w-7xl mx-auto bg-white rounded-full px-6 shadow-sm"
        ...
        >
          <div className="flex items-center h-[64px] gap-8">
                ...
                item.href ? (
                  ...
                  <div key={item.label} className="relative">
                    ...
                    <div
                      onMouseEnter={() => setActive(item.label)}
                      onMouseLeave={() => setActive(null)}
                      className={`absolute top-full left-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden transition-all duration-200 ${
                        active === item.label
                    ...
                    >
                      {item.children?.map((child) => (
                        <Link
                          key={child.href}
                          href={child.href}
                          className="flex flex-col px-5 py-4 border-b border-gray-50 last:border-0 hover:bg-gray-50 group transition-colors"
                        >
                          <span className="text-[14px] font-semibold text-gray-900 group-hover:text-gray-900 transition-colors">
                            {child.label}
                          </span>
                          <span className="text-[12px] text-gray-400 mt-0.5">{child.sub}</span>
                        </Link>
                      ))}
                    </div>
                  </div>
            ...
            <div className="hidden lg:flex items-center gap-3 ml-auto">
              <Link
                href="/login"
                className="text-[15px] font-medium text-gray-600 hover:text-gray-900 transition-colors px-3"
              >
                Log in
              </Link>
              <Link
        ...
        {mobileOpen && (
          <div className="lg:hidden max-w-7xl mx-auto mt-2 bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden">
            <div className="px-6 py-4 space-y-1">
              {navItems.map((item) =>
                item.href ? (
                  <Link
                    key={item.label}
                    href={item.href}
                    className="block px-4 py-3 text-[15px] font-medium text-gray-800 hover:bg-gray-50 rounded-xl transition-colors"
                  >
                    {item.label}
                  </Link>
                ) : (
                  <div key={item.label}>
                    <button
                      ...
                      className="w-full flex items-center justify-between px-4 py-3 text-[15px] font-medium text-gray-800 hover:bg-gray-50 rounded-xl transition-colors"
                    >
                      {item.label}
                      <ChevronDown
                        size={14}
                    ...
                    {mobileExpanded === item.label && (
                      <div className="ml-4 mt-1 space-y-1">
                        {item.children?.map((child) => (
                          ...
                          >
                            <span className="text-[14px] font-semibold text-gray-900">{child.label}</span>
                            <span className="text-[12px] text-gray-400 mt-0.5">{child.sub}</span>
                          </Link>
                        ))}
                      </div>
              ...
              <div className="pt-4 border-t border-gray-100 flex flex-col gap-3">
                <Link
                  href="/login"
                  className="block text-center px-4 py-3 text-[15px] font-medium text-gray-700 hover:bg-gray-50 rounded-xl transition-colors"
                >
                  Log in
                </Link>
                <Link
