...
export default function Stepper({
  ...
  return (
    <div className="outer-container" {...rest}>
      <div className={`step-circle-container ${stepCircleContainerClassName}`} style={{ border: '1px solid var(--border-color, #e2e8f0)' }}>
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: any) {
  ...
  const handleClick = () => {
    if (step !== currentStep && !disableStepIndicators) { if (step > currentStep && validateStep && !validateStep(currentStep)) return; onClickStep(step); }
  };
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'var(--step-inactive-bg)', color: 'var(--step-inactive-color)' },
          active: { scale: 1, backgroundColor: 'var(--step-active-bg)', color: 'var(--step-active-color)' },
          complete: { scale: 1, backgroundColor: 'var(--step-complete-bg)', color: 'var(--step-complete-color)' }
        }}
...
function StepConnector({ isComplete }: any) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: 'var(--step-connector-bg)' }
  };
